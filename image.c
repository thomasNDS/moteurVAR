...
int main(int argc, char *argv[]){
  ...
  char* indice = getparam(entries,m,"indice");
  char* kchar = getparam(entries,m,"nbAff");
  afficherBarreImage();
  ...
  printf("<h4>image %d </h4>",atoi(indice));
  printf ("<<div class=zoom><img src=\"http://mrim.imag.fr/voc10/images/%s\"",valList[atoi(indice)]);
  printf ("/></div> </br>\n");
  float ** dist =Recherche(moyConcept,seuil*10, trainDescriptors, valDescriptors,atoi(indice),nbConcepts, trainList,valList, concepts, trainAnns,k);
  //Affichage
  affichageResultat(concepts, k, nbConcepts, dist,trainList, moyConcept,seuil);
